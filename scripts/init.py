...
from vip.models import VipPermRalation
...
def init_permission():
    ...
    for name,desc in permissions :
        ...
        print('create permissoon:%s'%perm.name)
...
def create_vip_perm_relations():
    ...
    antlocation = Permission.objects.get(name='anylocation')
    unlimit_like = Permission.objects.get(name='unlimit_like')
    show_liked_me  = Permission.objects.get(name='show_liked_me')

    #一级会员
    VipPermRalation.objects.get_or_create(vid=vip1.id,pid=vipflag.id)
    VipPermRalation.objects.get_or_create(vid=vip1.id,pid=superlike.id)

    #二级会员
    VipPermRalation.objects.get_or_create(vid=vip2.id, pid=vipflag.id)
    VipPermRalation.objects.get_or_create(vid=vip2.id, pid=superlike.id)
    VipPermRalation.objects.get_or_create(vid=vip2.id, pid=rewind.id)

    #三级会员
    VipPermRalation.objects.get_or_create(vid=vip3.id, pid=vipflag.id)
    VipPermRalation.objects.get_or_create(vid=vip3.id, pid=superlike.id)
    VipPermRalation.objects.get_or_create(vid=vip3.id, pid=rewind.id)
    VipPermRalation.objects.get_or_create(vid=vip3.id, pid=antlocation.id)
    VipPermRalation.objects.get_or_create(vid=vip3.id, pid=unlimit_like.id)
    VipPermRalation.objects.get_or_create(vid=vip3.id, pid=show_liked_me.id)
